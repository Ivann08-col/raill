...
def update_usuario(usuario_id):
    try:
        ...
        # Preferencias (por ejemplo: { "habitos": { ... } })
        if 'preferencias' in data:
            try:
                usuario.preferencias = data.get('preferencias')
            except Exception:
                return jsonify({'error': 'Preferencias no válidas'}), 400
...
@usuario_bp.route('/<string:usuario_id>/preferencias', methods=['PUT', 'PATCH'])
@jwt_required()
def update_preferencias(usuario_id):
    try:
        usuario = Usuario.query.get(usuario_id)
        if not usuario:
            return jsonify({'error': 'Usuario no encontrado'}), 404
        ...
        # Guardar preferencias de forma segura
        usuario.preferencias = data.get('preferencias') if 'preferencias' in data else data
        db.session.commit()
        return jsonify(usuario.to_dict()), 200
    except Exception as e:
        db.session.rollback()
