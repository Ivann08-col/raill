...
export default function Profile({ defaultTab = 'info' }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await api.get(cfg.paths.me);
        setUsuario(res.data);
      } catch (e) {
      ...
      try {
        const res2 = await api.get(cfg.paths.tareas);
      ...
      try {
        const resp = await api.get(cfg.paths.misRecompensas);
  ...
  // Helpers to compute achievements and derive XP/level client-side
  const getNumber = (user, keys) => {
    for (const k of keys) {
      const v = user?.[k];
      if (v == null) continue;
  ...
  const computeAchievementsFromUser = (user) => {
    if (!user) return [];
    const sessions = getNumber(user, ['sesiones_totales', 'sesiones', 'sesiones_totales_count']);
    const pomodoros = getNumber(user, ['pomodoros', 'pomodoro_count', 'pomodoros_count']);
    const meditations = getNumber(user, ['meditaciones_count', 'meditaciones', 'meditadas_count']);
    const racha = getNumber(user, ['racha_actual', 'racha']);

    if (!sessions && !pomodoros && !meditations && (user?.xp == null || user.xp === 0)) return [];

    return [
      { nombre: 'Primer Pomodoro', icono: '🍅', descripcion: 'Completaste tu primer Pomodoro', unlocked: pomodoros >= 1 },
      { nombre: 'Meditador Novato', icono: '🧘', descripcion: 'Realizaste tu primera meditación', unlocked: meditations >= 1 },
      { nombre: 'Concentración 7 días', icono: '🔥', descripcion: 'Has mantenido la concentración 7 días seguidos', unlocked: racha >= 7 },
      { nombre: '100 Sesiones', icono: '🏆', descripcion: 'Completaste 100 sesiones', unlocked: sessions >= 100 },
      { nombre: 'Nivel 5', icono: '⭐', descripcion: 'Alcanzaste el nivel 5', unlocked: (user?.nivel || 0) >= 5 || Math.floor((user?.xp || 0) / 3000) + 1 >= 5 },
      { nombre: 'Explorador Zen', icono: '🌿', descripcion: 'Has probado diferentes técnicas', unlocked: meditations >= 5 }
    ];
  };

  // When usuario changes, compute client-side achievements (unless backend provides its own)
  useEffect(() => {
    try {
      const computed = computeAchievementsFromUser(usuario);
      if (computed && computed.length) setLogros(computed);
    } catch (e) {
      // ignore
    }
  }, [usuario]);

  // Prevent body scroll and layout shifts while any modal is open
  useEffect(() => {
    const anyOpen = showChangePassword || showDeleteConfirm || showLogoutAllConfirm;
    const prevOverflow = document.body.style.overflow;
    try {
      if (anyOpen) {
        // lock scroll and mark modal-open to disable background transforms
  ...
  }, [showChangePassword, showDeleteConfirm, showLogoutAllConfirm]);
  ...
  // Derive counts, XP and level (client-side estimate when backend xp not provided)
  const { sessions: _sessions, pomodoros: _pomodoros, meditations: _meditations } = (function(u){
    return {
      sessions: getNumber(u, ['sesiones_totales', 'sesiones', 'sesiones_totales_count']),
      pomodoros: getNumber(u, ['pomodoros', 'pomodoro_count', 'pomodoros_count']),
      meditations: getNumber(u, ['meditaciones_count', 'meditaciones', 'meditadas_count'])
    };
  })(usuario || {});

  const xpFromActivity = (_pomodoros * 50) + (_meditations * 30) + (_sessions * 10);
  const totalXP = (usuario?.xp != null) ? Number(usuario.xp) : xpFromActivity;
  const derivedLevel = 1 + Math.floor((totalXP || 0) / 3000);
  const xpInLevel = (totalXP || 0) % 3000;

  // visibleLogros: en Información solo mostramos hasta 3 (primero desbloqueados)
  const visibleLogros = (() => {
    if (!logros || !logros.length) return [];
    const sorted = [...logros].sort((a,b) => (b.unlocked ? 1 : 0) - (a.unlocked ? 1 : 0));
        ...
        <div className="profile-info">
          ...
          {/* Grid de información */}
            ...
            <div className="hover-card" style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {(visibleLogros && visibleLogros.length ? visibleLogros : []).map((logro, idx) => (
                  <div
                    key={idx}
                    style={{
                      display: 'flex',
                ...
                {(!visibleLogros || visibleLogros.length === 0) && (
                  <div className="no-achievements">Aún no tienes logros recientes</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div className="hover-card" style={{
              ...
              >
                {usuario?.descripcion ? usuario.descripcion : <span style={{ color: 'var(--text-tertiary)' }}>Añade una breve biografía sobre ti, ve a Editar perfil.</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="stats-root">
          ...
          <div className="stats-cards">
            <div className="stat-card">
              ...
              <div className="stat-value">{derivedLevel}</div>
            ...
            <div className="card large-card">
              ...
              <div className="card-body">
                <div className="level-info">
                  <div className="level-label">Nivel {derivedLevel}</div>
                  <div className="level-xp">{(xpInLevel || 0).toLocaleString()} / {levelCap(usuario)} XP</div>
                </div>
                <div className="level-bar">
                  <div className="level-fill" style={{ width: `${Math.min(100, ((xpInLevel || 0) / levelCap(usuario)) * 100)}%` }} />
                </div>
                <div className="level-footer">{Math.max(0, levelCap(usuario) - (xpInLevel || 0))} XP hasta el siguiente nivel</div>
              </div>
            </div>
